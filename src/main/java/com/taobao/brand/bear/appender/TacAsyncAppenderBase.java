...
/**
 * @author jinshuan.li 28/05/2018 22:15
 */
public class TacAsyncAppenderBase<E> extends UnsynchronizedAppenderBase<E> implements AppenderAttachable<E> {
    ...
    TacAsyncAppenderBase.Worker worker = new TacAsyncAppenderBase.Worker();
     ...
     * <p/>
     * <p>Note that only if the buffer is nearly full are events discarded. Otherwise, when the buffer is "not full" all
     * events are logged.
     *
     * @param eventObject
    ...
    /**
     * Pre-process the event prior to queueing. The base class does no pre-processing but sub-classes can override this
     * behavior.
     *
     * @param eventObject
     */
    ...
    public void start() {
        ...
        if (discardingThreshold == UNDEFINED) { discardingThreshold = queueSize / 5; }
    ...
    protected E takeEventFromQueue() {

        E e = null;
        try {
            e = this.blockingQueue.take();
    ...
    class Worker extends Thread {
        ...
        @Override
        public void run() {
            TacAsyncAppenderBase<E> parent = TacAsyncAppenderBase.this;
            AppenderAttachableImpl<E> aai = parent.aai;

            // loop while the parent is started
            while (parent.isStarted()) {

                E e = parent.takeEventFromQueue();
                if (e != null) {
